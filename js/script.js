...
const screensContainer = document.querySelector(".main-controls__views.screen");
...
const appData = {
  ...
  updateUI() {
    ...
    document.getElementById("total-count-other").value = this.allServicePrices;
...
// Исправленные обработчики событий с привязкой контекста
plusButton.addEventListener("click", () => appData.addScreen());
inputElement.addEventListener("input", (e) => appData.updateRollbackValue(e));
...
// Добавляем обработчики событий с правильным контекстом
buttons.forEach((btn) => {
  if (btn.id === "start") {
    btn.addEventListener("click", () => appData.startCalculation());
  } else if (btn.id === "reset") {
...
document.getElementById("cms-select").addEventListener("change", function (e) {
  const otherInput = document.getElementById("cms-other-input").parentElement;
